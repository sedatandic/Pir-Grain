...
def update_trade(trade_id: str, body: dict, user=Depends(non_accountant)):
    ...
    if "buyerPaymentDate" in data:
        ...
        inv_update = {"paymentDate": payment_date}
        if payment_date:
...
def set_buyer_payment(trade_id: str, body: dict, user=Depends(non_accountant)):
    ...
    if payment_date:
        # Update trade: set payment date, mark as completed and paid
        trades_col.update_one(
            {"_id": ObjectId(trade_id)},
            {"$set": {
        ...
        # Auto-generate incoming commission invoice in accounting
        ...
        # Check if auto-generated invoice already exists for this trade
        ...
        # Clear payment date, revert status
        trades_col.update_one(
            {"_id": ObjectId(trade_id)},
            {"$set": {
                "buyerPaymentDate": "",
